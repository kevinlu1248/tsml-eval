...
from tsml_eval.estimators.regression.transformations import R_DST
...
class R_DST_Ridge(BaseEstimator, RegressorMixin):
    """
    A wrapper class which use R_DST as a transformer, followed by a Ridge Regressor.

    Attributes
    ----------
    regressor : object
        A sklearn pipeline for RidgeCV with L2 regularization.
    transformer : object
    ...
    def _init_components(self):
        self.regressor = make_pipeline(
            ...
            RidgeCV(alphas=self.alphas_ridge, fit_intercept=self.fit_intercept),
        )
    ...
    def fit(self, X, y):
        ...
        self.regressor = self.regressor.fit(self.transformer.transform(X), y)
        return self
    ...
    def predict(self, X):
        ...
        return self.regressor.predict(self.transformer.transform(X))

    def score(self, X, y):
        """
        Compute the MSE for the input time series X.

        Perform the prediction on input time series and return the MSE
        score based on the output information.
        ...
        -------
        float
            MSE score on the input time series
