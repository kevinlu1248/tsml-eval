...
from convst.transformers._commons import euclidean, manhattan, squared_euclidean
...
class R_DST(BaseEstimator, TransformerMixin):
    ...
    Depending on the parameters and of the type of data (i.e. multivariate, variable
    length, etc.) given during the fit method, it will call the adapted transformer.
    ...
    .. [1] Antoine Guillaume et al, "Random Dilated Shapelet Transform: A new approach
        of time series shapelets" (2022).
    .. [2] Antoine Guillaume, "Time series classification with shapelets: Application to
        predictive maintenance on event logs" (2023).
    ...
    def __init__(
        ...
        if shapelet_lengths_bounds is None:
            ...
            raise ValueError(
                "Shapelets lengths bounds should be a 1D array with 2 values"
            )
        self.lengths_bounds_reduction = check_is_numeric(lengths_bounds_reduction)
        if self.lengths_bounds_reduction >= 1:
            raise ValueError(
                "lengths_bounds_reduction parameter should be in range [0,1["
            )
    ...
    def fit(self, X, y):
        ...
        Random shapelets are generated using the parameters
        ...
        elif self.transform_type == STR_MULTIVARIATE:
            self.shapelets_ = self.fitter(
                X,
                y,
                self.n_shapelets,
    ...
    def _auto_class(self, X):
        """
        Select the type of transformation automatically.
        ...
        """
        # [STR_UNIVARIATE,STR_MULTIVARIATE,STR_UNIVARIATE,STR_MULTIVARIATE_VARIABLE]
        if isinstance(X, list) or X.dtype == np.object_:
            ...
            X = check_array_3D(X)
            if X.shape[1] > 1:
                return STR_MULTIVARIATE
            else:
                return STR_UNIVARIATE

    def _set_fit_transform(self, X):
        """
        Initialize the fitter and transformer.
        ...
        if _type == STR_UNIVARIATE:
            ...
            from convst.transformers._univariate_variable_length import (
                U_VL_apply_all_shapelets,
                U_VL_generate_shapelet,
            )
            ...
            from convst.transformers._multivariate_variable_length import (
                M_VL_apply_all_shapelets,
                M_VL_generate_shapelet,
            )
    ...
    def _check_params(self, n_timestamps):
        ...
        Main checks are that shapelet length do not exceed time series lengths, and that
        we have at least 3 timestamp in all series.
        ...
        if not isinstance(self.n_shapelets, (int, np.integer)):
            raise TypeError(
                "'n_shapelets' must be an integer (got {}).".format(self.n_shapelets)
            )
        ...
        if not np.all(1 < shapelet_lengths):
            raise ValueError(
                "All the values in 'shapelet_lengths' must be "
                "greater than 1, but got {} ({} < 1).".format(
                    shapelet_lengths, shapelet_lengths.min()
                )
            )
        ...
        if not np.all(shapelet_lengths <= n_timestamps):
            if n_timestamps < 3:
                raise ValueError(
                    """Input data goint {} timestamps, at least 4 are requiered. Input \
                    format should be (n_samples, n_features, n_timestamps)""".format(
                        n_timestamps
                    )
                )
            else:
                warnings.warn(
                    """All values in 'shapelet_lengths' must be lower than or equal to \
                    'n_timestamps' (got {} > {}). Changed shapelet size to {}""".format(
                        shapelet_lengths.max(), n_timestamps, n_timestamps // 2
                    ),
                    stacklevel=2,
                )
    ...
    def _validate_transform_type(self, transform_type):
        ...
        valid = [
            ...
            STR_MULTIVARIATE,
            STR_UNIVARIATE,
            STR_MULTIVARIATE_VARIABLE,
        ]
        if transform_type not in valid:
            raise ValueError(
                "Wrong transform_type parameter value, got {}, valid are {}".format(
    ...
    def _validate_percentiles(self, percentiles):
        ...
        if percentiles.dtype == int or percentiles.dtype == float:
            if percentiles[0] <= percentiles[1]:
                return percentiles
        raise ValueError(
            "Wrong percentiles values, got {}, expected an array of size 2".format(
                percentiles
            )
        )
    ...
    def _validate_distances(self, distance_str):
        ...
        if distance_str not in valid:
            raise ValueError(
                "Wrong distance parameter value, got {}, valid ones are {}".format(
                    distance_str, valid
                )
            )
        return distance_str
